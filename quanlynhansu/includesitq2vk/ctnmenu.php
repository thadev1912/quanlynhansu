...
function ajaxLoad(url,id) /* tham so truyen vao la url va id*/
   {
           ...
           x.onreadystatechange = function()
                   {
                       ...
                       el.innerHTML='<div align=center><font color=white> BBĐ Cần Thơ....';
  ...
  function doShowCalendar()
{
	...
	if(ngay != '' && kenh != ''){
		...
		alert('Có lỗi trong quá trình hiển thị');
	}
} 
</script>
<!---  CODE CHÍNH TRONG MỤC MENU-->
...
<table border='0' width='254' name='vigfdsggfds'  cellSpacing=0 cellPadding=0>
	...
	<tr><td background='images/nhansu_10.png' width='71' height='23'></td>
		...
		<tr><td valign='top'>
			...
			<!--CODE AJAX LOAD TRANG -->
            <select name="select" id="kenh" class='quangvinhit' onchange="doShowCalendar();"  size="1" style="width: 80px; height:19px">	
				<?php
                	/*--LOAD DU LIEU NHAN SU +BO PHAN OKAY*/
   $query_tinh=@mysqli_query($conn,"select * from qlns_congty ORDER BY qlns_idct ASC");
...
?></select>
	...
	<td background='images/giaodientimkiem_41.png' width='67' height='28'>
	 <select name="select" id="ngay"  class='quangvinhit' onchange="doShowCalendar();" size="1" style="width: 66px; height:19px">
                   <?php 
                   	$query_tinh1=@mysqli_query($conn,"select * from qlns_bophan ORDER BY qlns_idbp ASC");
                   while($row_tinh1=@mysqli_fetch_array($query_tinh1))
                     {
