...
?></select>
	...
	<td background='images/giaodientimkiem_41.png' width='67' height='28'>
	 <select name="select" id="ngay"  class='quangvinhit' onchange="doShowCalendar();" size="1" style="width: 66px; height:19px">
                   <?php 
                   	$query_tinh1=@mysql_query("select * from qlns_bophan ORDER BY qlns_idbp ASC");
                     while($row_tinh1=@mysql_fetch_array($query_tinh1))
                     {
                     $qlns_tenbophan1=$row_tinh1["qlns_tenbophan"];
         	         $qlns_mabp1=$row_tinh1["qlns_mabp"];
         	     ?>    	  	 
                  <option value="<?php echo $qlns_mabp1; ?>" <?php if($qlns_mabp1==$bophan){echo "selected";}?>> <? echo $qlns_tenbophan1;?>
                  <?php    }
                   ?>
                   </select> 
	</td></tr> 
			...
			<?php 
			  ...
			  echo "<table border='0' width='229' name='vfdfasaffdgsdgsigfdsffggfds'  cellSpacing=0 cellPadding=0>";	  
			   ...
			   if($page=="") $page="1";
			   $sqlvinh=@mysql_query("select qlns_manv,qlns_mahsnv,qlns_honv,qlns_tennv,qlns_macv from qlns_hosonhanvien where qlns_mabp='$bophan' and qlns_tinhtrangnv = 1 order by qlns_hsnv desc");
			   $totalrow=@mysql_num_rows($sqlvinh);
			   $rowperpage=15;
			   $max_page=ceil($totalrow/$rowperpage);
			   $start=($page-1)* $rowperpage;
			   $sqlgioihan=@mysql_query("select qlns_manv,qlns_mahsnv,qlns_honv,qlns_tennv,qlns_macv from qlns_hosonhanvien where qlns_mabp='$bophan' and qlns_tinhtrangnv = 1 order by qlns_hsnv desc limit $start,$rowperpage");
