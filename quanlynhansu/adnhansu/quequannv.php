...
function pb_display(x,y)       
   {
   ajaxLoad('displayctnquequannv.php?lich='+x,'lichps');
   }
</script>
<center>
<form action="lylichnvctn.php?act=do"  method="post" NAME="mainform" onsubmit="return check_form(this)">
	...
	<tr> 
      	<td colspan="1"> 
        <div align="center"><font face=Tahoma size="2" color="#0000FF"><b>Tìm nhân viên theo Quê quán</b></font></div></td>
    	</tr>
	</TBODY></TABLE>
	...
	<TABLE  cellSpacing=2 cellPadding=2 width="95%" border=0>
    	...
    	<tr> 
      	<td><font size="2"><b>Họ Nhân viên:</b></font>
        <select name="title" id="title" onchange="pb_display(this.value);">
        
<?php $a="select * from qlns_hosonhanvien group by qlns_tinhthanhqq";
$result = @mysql_query($a) or die ("loi");
while ($row = @mysql_fetch_array($result))
{?><option value='<?=$row['qlns_tinhthanhqq']?>' <?php if($row['qlns_tinhthanhqq']=="Đà Nẵng") {echo "selected='select'"; } ?>><?php echo $row['qlns_tinhthanhqq']; ?></option>
<? }?> ;
</select></td>
    	</tr>
   <tr><td><div id="lichps">
