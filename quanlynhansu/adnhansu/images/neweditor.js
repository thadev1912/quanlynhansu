...
this.height = "auto";
this.border = 2;
this.bodyStyle = 'background-color: white; border-left: 1px solid gray ;border-right: 1px solid gray ;border-bottom: 1px solid gray ;font-family: "arial, helvetica, sans-serif"; font-size: 12px;';
...
/*  if (cmdID == 'popupeditor') {
  ...
  // FontStyle (change CSS className)
    ...
    var fontArray = editdoc.getElementByID(tags("FONT"));
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert image
		...
		///window.open('?page=image_library&category_id='+current_category_id+'&target='+objname,'image_library','toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=800,height=600')
 ...
 // src="' +_editor_url+ 'popups/blank.html"
  ...
  if (mode == "textedit" || editor_obj.tagName.toLowerCase() == 'iframe') {
    ...
    editor_obj.onkeypress  = function() { 
		var el,ky,shi
		el=event.srcElement;ky=event.keyCode;shi=event.shiftKey;
  ...
  //
    ...
    if (config.stylesheet) {
      html += '<link href="../richtext/%27%20+config.stylesheet+%20%27" rel="stylesheet" TYPE="text/css">\n';
    }
    html += '<STYLE>\n';
...
//      + ' oncontextmenu="parent.editor_cMenu_generate(window,\'' +objname+ '\');"'
    ...
    editdoc.onkeypress     = function() { 
		...
		el=editor_obj.contentWindow.event.srcElement;
		ky=editor_obj.contentWindow.event.keyCode;shi=editor_obj.contentWindow.event.shiftKey;
